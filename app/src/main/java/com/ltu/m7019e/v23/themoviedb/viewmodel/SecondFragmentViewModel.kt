...
class SecondFragmentViewModel (application: Application) : AndroidViewModel(application){
        ...
        getMoviesApiCall { movieList ->
            ...
            getGenresApiCall {genreList ->
                ...
                genreList.forEach { genre ->
                    val genreMovies = movieList.filter { it.movie_genres.contains(genre.id)}
                    if (!genreMovies.isEmpty()) {
                        // If no movies of the genre exist we remove the genreList
                        genre.movieList = genreMovies
                        listOfGenres.add(genre)
                    }
                }
                _genreList.postValue(listOfGenres)
            }
        }
    }
    ...
    private fun getMoviesApiCall(callback: (List<Movie>) -> Unit) {
        ...
        movieApiClient.getPopularMovies(page = 10) { movieList, error ->
            if (error != null ) {
